 ...
 * of tasks have been submitted further submissions will block until a task finishes executing.
 * <p>
 * Users can use this class to control the concurrency in different sections of their code to avoid overloading a shared
 * global thread pool.
 * <p>
 * <b>Note:</b> Using a {@code BoundedExecutorService} does not guarantee the maximum number of threads generated by the
 * underlying executor service, as this class has no control over the behavior or implementation of the supplied
 ...
 * <p>
 * Consider an executor service with an unrestricted thread pool. Let's also assume that this executor service has been
 * optimized to initiate task execution immediately upon arrival. In this scenario, it's possible that a new thread is
 * generated, at some point between the completion of a preceding task and the start of the execution of an incoming
 * task. For example if the thread executing a previous task must finish some cleanup operations after the task
 * completes. In practical terms a {@code BoundedExecutorService} does significantly impact the total number of active
 * threads in an executor service.
 * 
